...
from rldm.utils import football_tools_ppo as ft
...
Example usage:
    ...
    NOTE:
        There are a few baseline checkpoints available for you to test:
          -> /mnt/logs/baselines/PPO_3_vs_3/checkpoint_009800/checkpoint-9800
        Test them:
          python -m rldm.scripts.evaluate_checkpoint -c /mnt/logs/baselines/PPO_3_vs_3/checkpoint_009800/checkpoint-9800
          
          python -m rldm.scripts.evaluate_checkpoint_ppo -c /mnt/logs/baselines/baseline_1/checkpoint_0/checkpoint-0
"""
...
def main(checkpoint, algorithm, env_name, config, num_episodes, debug):
    ...
    path = "/mnt/rldm/scripts/ppo" + str(baseline_num) + "_rewards_total.npy"
    np.save(path, rewards_total)
    print("\nSaved to npy", rewards_total)

    pass_attempts = eps_stats["pass_attempts"]
    path = "/mnt/rldm/scripts/ppo" + str(baseline_num) + "_pass_attempts.npy"
    np.save(path, pass_attempts)
    print("\nSaved to npy", pass_attempts)

    shoot_attempts = eps_stats["shoot_attempts"]
    path = "/mnt/rldm/scripts/ppo" + str(baseline_num) + "_shoot_attempts.npy"
    np.save(path, shoot_attempts)
    print("\nSaved to npy", shoot_attempts)

    num_success_pass = eps_stats["num_success_pass"]
    path = "/mnt/rldm/scripts/ppo" + str(baseline_num) + "_num_success_pass.npy"
    np.save(path, num_success_pass)
    print("\nSaved to npy", num_success_pass)

    mean_values = np.array(mean_stats)
    path = "/mnt/rldm/scripts/ppo" + str(baseline_num) + "_mean_stats.npy"
    np.save(path, mean_values[[2,4,5,6]])
